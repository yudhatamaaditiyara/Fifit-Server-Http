...
class Server
{
	...
	start(){
		if (this.isStarted) {
			return Promise.reject(new RuntimeError('The server is started'));
		}
		return new Promise((resolve, reject) => {
			this.resource.once('error', reject);
			this.resource.listen(this.options.port, this.options.host, () => {
	...
	stop(){
		if (!this.isStarted) {
			return Promise.reject(new RuntimeError('The server is not started'));
		}
