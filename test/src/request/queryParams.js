...
describe('Request#queryParams', () => {
  it('must be typeof Object', () => {
    let request = Object.create(Request.prototype);
    request.url = 'http://hostname/path?query=value';
    assert.ok(request.queryParams !== null && typeof request.queryParams === 'object');
  });

  it('must be Object.create(null) when fail to parse url', () => {
    let request = Object.create(Request.prototype);
    request.url = null;
    assert.ok(request.queryParams !== null && typeof request.queryParams === 'object');
  });
  ...
  it('must be work with request url "/"', (done) => {
    ...
    server.listen((request, response) => {
      response.end(querystring.encode(request.queryParams));
    });
    server.start().then(() => {
