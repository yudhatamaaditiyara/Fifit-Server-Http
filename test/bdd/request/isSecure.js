...
describe('Request#isSecure', () => {
	...
	/**
	 */
	it('must be createSecureServerIpv6Host() -> isSecure === true', (done) => {
		let server = helper.createSecureServerIpv6Host();
		server.listen((request, response) => {
			response.end(request.isSecure ? 'true' : 'false');
		});
		server.start().then(() => {
	...
	/**
	 */
	it('must be createSecureServerIpv6HostDefaultPort() -> isSecure === true', (done) => {
		let server = helper.createSecureServerIpv6HostDefaultPort();
		server.listen((request, response) => {
			response.end(request.isSecure ? 'true' : 'false');
		});
		server.start().then(() => {
